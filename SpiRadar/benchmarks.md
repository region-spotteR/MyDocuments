...
Thus our sequence needs to stop at some *t* &lt; 2*π*. The optimal *t* is such that all column name are plotted with even distance to each other. It turn out that we can calculate *t* using a constant *k* such that *t* = *k* \* 2 \* *π* &lt; 2 \* *p**i* (meaning ⇒*k* &lt; 1). This way we end up with $k=, where *n* is total count of all columns used for comparison.
